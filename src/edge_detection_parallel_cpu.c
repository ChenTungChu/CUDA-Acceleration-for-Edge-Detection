/**
 * @file edge_detection_parallel_cpu.c
 * @author jz544 & cc2396
 * @brief 
 * @version 1.0
 * @date 2022-04-30
 * 
 * compile: gcc -o edge_detection_parallel_cpu edge_detection_parallel_cpu.c -lm -fopenmp
 * run: ./edge_detection_parallel_cpu 100 10 # 100 is the number of images it processes, 10 is the number of threads
 */
...
void sobel_edge_filter_omp(pgm* image, pgm* out_image);

int main(int argc, char *argv[]) {
	pgm image, out_image;   // define the input image and output image
	int num_of_imgs, num_of_threads;

	if (argc == 1) {
		printf("[INFO] no arguments, use default number of images\n");
		printf("[INFO] number of images: %s, number of threads: %s\n", argv[1], argv[2]);
	...
	for (int i = 0; i < num_of_imgs; i++) {
        sobel_edge_filter_omp(&image, &out_image);
	}
	// kernel
...
void sobel_edge_filter_omp(pgm* image, pgm* out_image) {
	...
	//{
		#pragma omp parallel for default(shared) private( i, j, gx, gy)
		for( i = 1; i < image->height - 2; i++ )
		{	//printf("%d\n", omp_get_thread_num());
			for( j = 1; j < image->width - 2; j++ )
			{
				gx = image->imageData[i-1][j-1]*sobel_x[0][0] + image->imageData[i-1][j]*sobel_x[0][1] + image->imageData[i-1][j+1]*sobel_x[0][2]+
					 image->imageData[i][j-1]*sobel_x[1][0] + image->imageData[i][j]*sobel_x[1][1] + image->imageData[i][j+1]*sobel_x[1][2]+
					 image->imageData[i+1][j-1]*sobel_x[2][0] + image->imageData[i+1][j]*sobel_x[2][1] + image->imageData[i+1][j+1]*sobel_x[2][2];
				gy = image->imageData[i-1][j-1]*sobel_y[0][0] + image->imageData[i-1][j]*sobel_y[0][1] + image->imageData[i-1][j+1]*sobel_y[0][2]+
					 image->imageData[i][j-1]*sobel_y[1][0] + image->imageData[i][j]*sobel_y[1][1] + image->imageData[i][j+1]*sobel_y[1][2]+
					 image->imageData[i+1][j-1]*sobel_y[2][0] + image->imageData[i+1][j]*sobel_y[2][1] + image->imageData[i+1][j+1]*sobel_y[2][2];
